...
export interface IMessage{
	...
	msg_key:string; // usually the id of the aggregate (used for partitioning)
	msg_topic:string;
...
export interface IDomainMessage extends IMessage{
	...
	aggregate_id:string; // id of the source/target aggregate (source if event, target if command)
...
export abstract class DomainMsg implements IDomainMessage{
	...
	msg_name:string = (<any>this).__proto__.constructor.name;

	abstract msg_type: MessageTypes
	abstract msg_key:string; // usually the id of the aggregate (used for partitioning)
	abstract msg_topic:string;

	abstract aggregate_id: string;
